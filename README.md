...
iPhone の Safari で使える静的 Web カメラです。撮影枠と保存する JPEG は縦向きの黄金比（幅:高さ＝1:1.618）です。右下の角から内側へ巻く黄金螺旋を初期表示し、グリッドにも切り替えられます。ガイドはプレビュー専用で、保存する写真には入りません。前後カメラの切替にも対応し、画像は端末内だけで処理します。iPhone 14 Plus と iPhone 17e を含め、画面幅に合わせて表示が変わります。
